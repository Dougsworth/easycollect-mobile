...
import type { LocalIntent, AiChatUsage } from '@/shared/types/app.types';
...
export async function executeLocalQuery(intent: LocalIntent, landlordId: string): Promise<string> {
  switch (intent) {
    case 'overdue': {
      ...
      if (overdue.length === 0) return 'Great news! No tenants are currently overdue.';
    ...
    case 'recent_payments': {
      ...
      if (payments.length === 0) return 'No recent payments found.';
