...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signUp = async (
    ...
    // RN adaptation: use deep link scheme instead of window.location.origin
    const { error } = await supabase.auth.signUp({
      email,
      password,
      options: {
        data: metadata,
        emailRedirectTo: 'easycollect://email-verified',
      },
    });

    // Fire-and-forget welcome email for new landlords
    if (!error && metadata.role === 'landlord') {
      fetch(`${process.env.EXPO_PUBLIC_SUPABASE_URL}/functions/v1/send-landlord-welcome`, {
        method: 'POST',
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      if (!data.url) return { error: 'Failed to get auth URL' };
